      ...
      <div className="container max-w-3xl mx-auto px-4 sm:px-6 space-y-10 text-slate-700 leading-relaxed">
        ...
        {post.sections && post.sections.length > 0 ? (
          post.sections.map((section, idx) => (
            <FadeUp key={idx} delay={0.08 + (idx % 3) * 0.05}>
      ...
      {relatedStories.length > 0 && (
        <div className="container max-w-5xl mx-auto px-4 sm:px-6 mt-20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {relatedStories.map((rel, index) => (
              <FadeUp key={rel._id || rel.id} delay={0.08 + index * 0.08}>
                <article
                  className="bg-white rounded-lg border border-slate-200/90 shadow-sm overflow-hidden flex flex-col justify-between transition group h-full"
                >
                  <div>
                  ...
                  <div className="px-4 sm:px-5 pb-4 pt-2 border-t border-slate-100 flex items-center justify-between mt-auto">
