    ...
    <div className="space-y-6 sm:space-y-7 max-w-[1600px] mx-auto pb-12 text-slate-800">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
        <div>
          <div className="flex items-center gap-2.5 sm:gap-3">
            <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-800 tracking-tight">Recycle Bin</h3>
            <span className="px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full bg-rose-50 text-[#6B1527] font-bold text-[10px] sm:text-xs border border-rose-100">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
          ...
          {items.length > 0 && (
            <button
              ...
              className="flex items-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl bg-white hover:bg-rose-50 border-2 border-[#6B1527] text-[#6B1527] text-xs sm:text-sm md:text-base font-semibold shadow-sm transition-all duration-300 cursor-pointer"
            >
        ...
        <div className="overflow-x-auto">
          {filteredItems.length === 0 ? (
            <div className="py-12 sm:py-20 text-center px-4">
              <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 mx-auto mb-3">
                <FiTrash2 className="text-2xl sm:text-3xl" />
              </div>
              <h4 className="text-sm sm:text-base font-bold text-slate-800">Recycle Bin is Empty</h4>
            ...
            <table className="w-full text-left border-collapse text-xs sm:text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold uppercase tracking-wider text-[11px] sm:text-xs">
                  <th className="py-3 sm:py-4 px-3 sm:px-6">Item Details</th>
                  <th className="py-3 sm:py-4 px-3 sm:px-6">Type</th>
                  <th className="py-3 sm:py-4 px-3 sm:px-6">Original Info</th>
                  <th className="py-3 sm:py-4 px-3 sm:px-6">Deleted On</th>
                  ...
                  return (
                    <tr key={item._id || item.id} className="hover:bg-slate-50/70 transition">
                        ...
                        <div className="flex items-center gap-2.5 sm:gap-3.5">
                          {item.image ? (
                            <img
                              ...
                              className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl object-cover bg-slate-100 border border-slate-200 shrink-0"
                            />
                          ) : (
                            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 shrink-0">
                              <Icon className="text-lg sm:text-xl" />
                            </div>
                          )}
                          <div className="min-w-0 max-w-[130px] sm:max-w-xs">
                            <span className="font-bold text-xs sm:text-sm md:text-base text-slate-800 block truncate">{item.itemTitle}</span>
                            <span className="text-[11px] sm:text-xs text-slate-400 block mt-0.5 truncate">
                      ...
                      <td className="py-3 sm:py-4 px-3 sm:px-6 whitespace-nowrap">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full font-bold text-[10px] sm:text-xs ${typeConfig.bg}`}
                        >
                          <Icon className="text-xs sm:text-sm" />
                          <span>{typeConfig.label}</span>
                        </span>
                      </td>
                      <td className="py-3 sm:py-4 px-3 sm:px-6 text-slate-700 font-medium text-xs sm:text-sm whitespace-nowrap">
                        {item.itemSubtitle || "—"}
                      </td>
                      <td className="py-3 sm:py-4 px-3 sm:px-6 text-slate-500 text-[11px] sm:text-xs font-medium whitespace-nowrap">{formattedDate}</td>
                      <td className="py-3 sm:py-4 px-3 sm:px-6 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end gap-1.5 sm:gap-2.5">
                          <button
                            onClick={() => handleRestore(item)}
                            className="inline-flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-800 font-bold text-xs transition border border-emerald-200 cursor-pointer"
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-800 font-bold text-xs transition border border-rose-200 cursor-pointer"
      ...
      {itemToDeletePermanent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/50 backdrop-blur-xs">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-5 sm:p-7 border border-slate-200 animate-in fade-in zoom-in-95">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-rose-50 flex items-center justify-center text-rose-600 mb-3.5 sm:mb-4">
              <FiAlertTriangle className="text-2xl sm:text-3xl" />
            </div>
            <h4 className="text-lg sm:text-xl font-bold text-slate-900">Permanently Delete Item?</h4>
            <p className="text-xs sm:text-sm text-slate-600 mt-1.5 mb-5 sm:mb-6 leading-relaxed">
            ...
            <div className="flex items-center gap-2.5 sm:gap-3">
              <button
                ...
                className="flex-1 py-2.5 sm:py-3 rounded-xl border border-slate-200 text-xs sm:text-sm font-semibold text-slate-700 hover:bg-slate-50 transition cursor-pointer"
              >
                Cancel
              </button>
              <button
                onClick={() => handleDeletePermanent(itemToDeletePermanent)}
                className="flex-1 py-2.5 sm:py-3 rounded-xl bg-rose-600 text-white text-xs sm:text-sm font-semibold hover:bg-rose-700 transition shadow-xs cursor-pointer"
              >
                Delete Permanently
              </button>
            </div>
      ...
      {confirmEmptyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/50 backdrop-blur-xs">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-5 sm:p-7 border border-slate-200 animate-in fade-in zoom-in-95">
            ...
            <h4 className="text-lg sm:text-xl font-bold text-slate-900">Empty Entire Recycle Bin?</h4>
            <p className="text-xs sm:text-sm text-slate-600 mt-1.5 mb-5 sm:mb-6 leading-relaxed">
