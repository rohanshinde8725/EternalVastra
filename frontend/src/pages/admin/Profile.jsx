...
const Profile = () => {
  ...
  return (
    <div className="space-y-5 sm:space-y-7 max-w-[1000px] mx-auto pb-12 text-slate-800">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-7 border border-slate-200/90 shadow-sm space-y-5 sm:space-y-7 text-xs sm:text-sm">
        {/* Profile Card Header */}
        <div className="flex flex-col sm:flex-row sm:items-center gap-3.5 sm:gap-5 border-b border-slate-200 pb-5 sm:pb-6">
          <img
            src={profile.avatar}
            alt={profile.name}
            onError={(e) => {
            ...
            className="w-14 h-14 sm:w-18 sm:h-18 rounded-full object-cover border-2 border-rose-200 shrink-0"
          />
          <div>
            <h4 className="text-base sm:text-lg font-bold text-slate-900">{profile.name}</h4>
            <span className="text-xs sm:text-sm text-slate-500 font-medium block">{profile.role}</span>
            <span className="text-[10px] sm:text-xs font-bold px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full bg-rose-50 text-[#6B1527] inline-block mt-1 sm:mt-1.5 border border-rose-100">
              Super Admin Access
            </span>
          </div>
        </div>
        ...
        {/* Password Security */}
        <div className="pt-2 sm:pt-3 border-t border-slate-200 space-y-3.5 sm:space-y-4">
          <h5 className="font-bold text-slate-900 text-sm sm:text-base">Security & Password</h5>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-5">
            <div>
        ...
        <div className="flex justify-end pt-2 sm:pt-3">
          <button
            type="submit"
            disabled={saving}
            className="w-full sm:w-auto justify-center flex items-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl bg-[#6B1527] hover:bg-white border-2 border-[#6B1527] text-white text-xs sm:text-sm md:text-base font-semibold shadow-sm transition-all duration-300 hover:text-[#6B1527] cursor-pointer disabled:opacity-50"
          >
