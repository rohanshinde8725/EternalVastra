  ...
  return (
    <div className="space-y-6 sm:space-y-7 max-w-[1600px] mx-auto pb-12 text-slate-800">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
        <div>
          <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-800 tracking-tight">Saree Categories & Weaves</h3>
          ...
          >
            <div className="relative h-40 sm:h-48 bg-slate-100 overflow-hidden">
              ...
              <div className="absolute bottom-3 sm:bottom-4 left-4 sm:left-5 right-4 sm:right-5 text-white">
                <div className="flex items-center justify-between">
                  <h4 className="text-base sm:text-lg font-bold drop-shadow-sm truncate">{cat.name}</h4>
                  <span className="text-[10px] sm:text-xs font-bold px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full bg-white/20 backdrop-blur-xs shrink-0 ml-2">
                    {cat.share || "15%"} sales
                  </span>
                </div>
                <span className="text-xs sm:text-sm text-amber-200 font-semibold block mt-0.5">{cat.count || 24} Sarees Live</span>
              </div>
            </div>
            ...
            <div className="p-4 sm:p-5 flex flex-col justify-between flex-1">
              ...
              <div className="mt-4 sm:mt-5 pt-3 border-t border-slate-100 flex items-center justify-between">
                <span className="text-[11px] sm:text-xs text-slate-500 font-mono font-semibold truncate">/{cat.slug || cat.name?.toLowerCase()}</span>
                <div className="flex items-center gap-1.5 shrink-0">
                  <button
                    onClick={() => handleDelete(cat)}
                    className="p-1.5 sm:p-2 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition cursor-pointer"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/50 backdrop-blur-xs">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-4 sm:p-7 border border-slate-200 animate-in fade-in zoom-in-95 max-h-[90vh] overflow-y-auto custom-admin-scroll">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3 sm:pb-4 mb-4 sm:mb-5">
              <h4 className="text-lg sm:text-xl font-bold text-slate-900">Add Saree Category</h4>
              <button
            ...
            <form onSubmit={handleAddCategory} className="space-y-3.5 sm:space-y-4 text-xs sm:text-sm">
              <div>
